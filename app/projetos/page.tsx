...
export default function ProjectsPage(){return <main>
  <header className="siteHeader"><Link className="wordmark" href="/">Rafaela Arantes</Link><nav><Link href="/">Início</Link><Link href="/formacao">Formação</Link><Link href="/depoimentos">Depoimentos</Link><a href="mailto:rafaaranteswork@gmail.com">Contato</a><Link className="languageSwitch" href="/en/projects">EN</Link></nav></header>
  ...
  <section className="projectsArchive">
    {projects.map((project,index)=><Link className="archiveCard" href={`/projetos/${project.slug}`} key={project.slug}>
      <div className="archiveImage">{project.coverKind==="pdf"?<iframe src={`${project.cover}#page=1&view=Fit&zoom=page-fit&toolbar=0`} title={`Prévia do projeto ${project.title}`} tabIndex={-1}/>:<img src={project.cover} alt={`Capa do projeto ${project.title}`}/>}<span>Ver projeto ↗</span></div>
  ...
  <section className="contactClean compactContact"><p className="eyebrow">Tem um projeto em mente?</p><h2>Vamos construir algo que faça sentido.</h2><a href="mailto:rafaaranteswork@gmail.com">Entrar em contato ↗</a></section>

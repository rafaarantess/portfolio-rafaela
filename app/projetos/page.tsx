...
export default function ProjectsPage(){return <main>
  ...
  <section className="projectsArchive">
    {projects.map((project,index)=><Link className="archiveCard" href={`/projetos/${project.slug}`} key={project.slug}>
      <div className="archiveImage">{project.coverKind==="pdf"?<iframe src={`${project.cover}#page=1&view=FitH&toolbar=0`} title={`Prévia do projeto ${project.title}`} tabIndex={-1}/>:<img src={project.cover} alt={`Capa do projeto ${project.title}`}/>}<span>Ver projeto ↗</span></div>
